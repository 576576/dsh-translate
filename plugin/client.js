 ...
 *    选中行持久高亮（同加号菜单）；仅翻译模式会话显示；关闭「允许源目标相同」时互相排除
 *  - 风格设置面板（conversation.input.overlay 左上锚点）：与加号/斜杠菜单天然互斥、
 *    点击输入框不收回、开始键入即收回、修改自动保存全局生效（无按钮）；支持风格预设
 *    保存/加载/删除
 *  - 设置页（settings.section）：「翻译模式」——首选项源/目标语言、允许源和目标语言相同、
 *    风格预设管理
 *  - i18n：locale 字典覆盖插件列出的全部 18 种语言（含交换按钮提示、设置页与预设文案），
 *    未命中回退中文
 */
...
const DICTS = {
  zh: {
    ...
    applyPreset: '应用',
    deletePreset: '删除该预设',
    noPresets: '暂无预设', editPreset: '编辑预设', saveChanges: '保存修改', clearAllPresets: '清除所有预设'
  },
  en: {
    auto: 'Auto',
...
return {
  ...
  apply(ctx) {
    ...
    const CSS = [
      ...
      '.dtr-preset-chip{display:inline-flex;align-items:center;gap:4px;height:26px;max-width:100%;padding:0 6px 0 12px;border-radius:14px;background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);font-size:12px;line-height:18px;cursor:pointer;user-select:none;-webkit-user-select:none;border:1px solid transparent;box-sizing:border-box}',
      '.dtr-preset-chip:hover{background:var(--dsw-alias-interactive-bg-hover-solid)}',
      '.dtr-preset-chip.dtr-chip-active{background:var(--dsw-alias-bg-module-platform);border-color:var(--dsw-alias-brand-primary)}',
      ...
      '.dtr-preset-edit{display:flex;flex-direction:column;gap:6px;border:1px solid var(--dsw-alias-border-inverted);border-radius:10px;padding:8px}',
      '.dtr-preset-edit-style{width:100%;box-sizing:border-box;min-height:56px;max-height:120px;resize:vertical;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-inverted);border-radius:8px;outline:none;padding:5px 8px;font-size:12px;font-family:inherit}',
      '.dtr-preset-edit-style:focus{border-color:var(--dsw-alias-brand-primary)}',
      '.dtr-preset-edit-actions{display:flex;justify-content:flex-end}',
      '.dtr-preset-clear{display:flex;justify-content:flex-end}',
      '.dtr-preset-clear-btn{color:var(--dsw-alias-state-error-primary)}',
    ...
    function StylePresets({ variant }) {
      ...
      const startEdit = (p) => {
        setEditId(p.id);
        setEditName(p.name);
        setEditStyle(p.style);
      };
      const saveEdit = () => {
        if (!editName.trim()) return;
        const cur = getState();
        const list = (cur.stylePresets || []).map((p) => p.id === editId ? { ...p, name: editName.trim(), style: editStyle } : p);
      ...
      // 设置页内：预设为胶囊（点击选中编辑，× 删除）+ 编辑区（保存修改）+ 清除所有预设
      const selected = presets.find((p) => p.id === editId) || null;
      return React.createElement('div', { className: 'dtr-presets' },
        presets.length === 0
          ? React.createElement('div', { className: 'dtr-desc' }, t('noPresets'))
          : React.createElement('div', { className: 'dtr-preset-chips' },
              presets.map((p) => React.createElement('span', {
                key: p.id,
                className: 'dtr-preset-chip' + (p.id === editId ? ' dtr-chip-active' : ''),
                title: p.style,
                onClick: () => startEdit(p)
              },
                React.createElement('span', { className: 'dtr-preset-chip-name' }, p.name),
                React.createElement('button', {
                  ...
                  onClick: (e) => {
                    e.stopPropagation();
                    removePreset(p.id);
                    if (p.id === editId) setEditId(null);
                  }
                }, '×')
              ))
            ),
        selected
          ? React.createElement('div', { className: 'dtr-preset-edit' },
              React.createElement('div', { className: 'dtr-desc' }, t('editPreset')),
              React.createElement('input', {
                className: 'dtr-preset-name-input',
                value: editName,
                placeholder: t('presetNamePlaceholder'),
              ...
              React.createElement('textarea', {
                className: 'dtr-preset-edit-style',
                value: editStyle,
                placeholder: t('stylePlaceholder'),
                onChange: (e) => setEditStyle(e.target.value)
              }),
              React.createElement('div', { className: 'dtr-preset-edit-actions' },
                React.createElement('button', {
                  type: 'button',
                  className: 'dtr-preset-btn',
          ...
          : null,
        React.createElement('div', { className: 'dtr-preset-clear' },
          React.createElement('button', {
            type: 'button',
            className: 'dtr-preset-btn dtr-preset-clear-btn',
